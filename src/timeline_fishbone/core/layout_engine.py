...
class SmartLayoutEngine:
    ...
    def __init__(self, layout_config: LayoutConfig, time_config: TimeLogicConfig):
        """
    ...
    def calculate_layout(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if self.config.smart_spacing:
            ...
            required_width = max_nodes * 2.5  # ~2.5cm per node horizontally
            
            adjusted_spacing = max(
                self.config.min_year_spacing,
                min(required_width, base_spacing)
            )
            ...
            adjusted_branch = max(
                base_branch,
                max_nodes * 0.6  # ~0.6cm vertical space per node
            )
            ...
            logger.debug(
                f"调整后间距: spacing={adjusted_spacing:.2f}cm, "
        ...
        layout_params['total_width'] = (
            (len(years) - 1) * layout_params['adjusted_spacing'] if len(years) > 1 else 0
        )
        
        logger.info(f"布局计算完成: 总宽度 {layout_params['total_width']:.2f}cm")
        
        return layout_params
    
    def get_year_position(self, year: int, layout_params: Dict[str, Any]) -> float:
        """
        ...
        Returns:
            X-coordinate in cm
        """
        return layout_params['positions'].get(year, 0.0)
