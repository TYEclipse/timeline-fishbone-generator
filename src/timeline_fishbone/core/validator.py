...
class DataValidator:
    ...
    # Valid categories - supports both predefined and custom categories
    VALID_CATEGORIES = {
        "singleproto",
        "multiproto",
        "adaptive",
        ...
        "attention",
        "hybrid",
        # Additional categories from sample data
        "CAM生成",
        "自监督聚类",
        "视觉语言模型",
        "视觉基础模型",
    ...
    def validate_columns(cls, df: pd.DataFrame) -> None:
        ...
        if missing_cols:
            ...
            required_str = ", ".join(cls.REQUIRED_COLUMNS)
            raise ValidationError(
                f"缺少必需的列: {missing_str}。" f"必需列: {required_str}"
            )
        logger.debug("列验证通过")
    ...
    def validate_null_values(cls, df: pd.DataFrame) -> None:
        ...
        if null_mask.any():
            ...
            null_rows_1based = [r + 1 for r in null_rows]
            raise ValidationError(
                f"以下行存在空值: {null_rows_1based}。"
                "所有必需列都必须有值。"
            )
        logger.debug("空值验证通过")
    ...
    def validate_categories(cls, df: pd.DataFrame) -> None:
        """
        Validate that categories column exists and has valid values.
        ...
        invalid_categories = categories - cls.VALID_CATEGORIES
        if invalid_categories:
            invalid_str = ", ".join(sorted(invalid_categories))
            valid_str = ", ".join(sorted(cls.VALID_CATEGORIES))
            raise ValidationError(
                f"无效的种类: {invalid_str}。" f"有效种类: {valid_str}"
            )

        cat_str = ", ".join(sorted(categories))
        logger.info(f"检测到 {len(categories)} 个种类: {cat_str}")
        logger.debug("种类验证通过")
    ...
    def validate_dataframe(cls, df: pd.DataFrame) -> None:
        ...
        logger.info(
            f"数据验证成功: {len(df)} 条记录，{df['年份'].nunique()} 个年份"
        )
    ...
    def load_and_validate(cls, file_path: Union[str, Path]) -> pd.DataFrame:
        ...
        try:
            if path.suffix.lower() == ".csv":
                ...
                fmt = path.suffix
                raise ValueError(
                    f"不支持的文件格式: {fmt}。" "请使用 .csv 或 .json 文件"
                )
